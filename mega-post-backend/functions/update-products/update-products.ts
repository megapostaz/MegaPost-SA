...
async function getAccessToken(credentialId: string, credentialSecret: string) {
  const authUrl = "https://api.amazon.co.uk/auth/o2/token"; // نفس المصري
...
async function getAmazonItemsBatch(asins: string[], config: any) {
  ...
  const payload = JSON.stringify({
    ...
    "marketplace": "www.amazon.sa", // 👈 سعودي
    "resources": ["offersV2.listings.price"]
  });

  const headers: Record<string, string> = {
    "Content-Type": "application/json; charset=utf-8",
    "x-marketplace": "www.amazon.sa", // 👈 سعودي
...
async function startAutoUpdate() {
  console.log("--- 🏁 بدء دورة التحديث بنظام Creators ---");
    ...
    try {
      ...
      for (const p of products) {
        ...
        if (newPrice !== undefined && Math.floor(newPrice) !== Math.floor(oldPrice)) {
          console.log(`⚠️ تم اكتشاف تغيير: ${p.asin} | قديم: ${oldPrice} | جديد: ${newPrice}`);
