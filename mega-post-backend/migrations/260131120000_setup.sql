...
DECLARE
    ...
    current_chat text; 
BEGIN
    -- ضبط وقت الانتظار للطلبات الخارجية
    PERFORM set_config('http.timeout_msec', '60000', true);

    -- جلب المنتجات المجدولة التي حان وقت نشرها (توقيت الرياض)
    FOR item IN
        SELECT * FROM products
        WHERE published_at IS NULL
        AND scheduled_at <= (NOW() AT TIME ZONE 'Asia/Riyadh')
        ORDER BY scheduled_at ASC 
        LIMIT 10  -- تم رفع الليميت لمعالجة منشورات أكثر في الدورة الواحدة
    ...
    LOOP
        ...
        BEGIN
            ...
            -- التحقق من وجود سعر للمنتجات العادية
            ...
            final_price := ROUND((COALESCE(item.price, 0) * (1 - (COALESCE(item.extra_payment_discount, 0)::float / 100)))::numeric, 2);
            old_price_val := ROUND(COALESCE(item.old_price, 0)::numeric, 2);
            v_clean_title := replace(replace(COALESCE(item.title, ''), '<', '&lt;'), '>', '&gt;');

        EXCEPTION WHEN OTHERS THEN 
            RAISE NOTICE 'Error in update logic for ASIN %', item.asin;
        END;

        -- 2. بناء نص المنشور (Template Logic)
        ...
        ELSE
            ...
            CASE COALESCE(item.template_id, 0)
                WHEN 0 THEN
                    -- إذا كان مجمع، استخدم النص المخزن في affiliate_link مباشرة
                ...
                WHEN 2 THEN
                    final_text := '💎 ' || v_clean_title || chr(10) || chr(10) || '🏷️ السعر الآن: <b>' || TO_CHAR(final_price, 'FM999990.00') || ' ر.س فقط</b>' || chr(10) || chr(10) || 'لينك العرض :' || chr(10) || item.affiliate_link;
        ...
        -- 3. النشر في قنوات تليجرام المحددة
        ...
        LOOP
            IF current_chat IS NOT NULL AND current_chat <> '' THEN
                BEGIN
                    ...
                    END IF;
                EXCEPTION WHEN OTHERS THEN 
                    RAISE NOTICE 'Telegram Publication Failed for Chat %', current_chat; 
                END;
            END IF;
        END LOOP;
        ...
        IF set.fb_page_id IS NOT NULL AND set.fb_page_id <> '' AND real_fb_token IS NOT NULL THEN
            BEGIN
                ...
                END IF;
            EXCEPTION WHEN OTHERS THEN 
                RAISE NOTICE 'Facebook Publication Failed'; 
            END;
        END IF;

        -- 5. تحديث حالة المنتج إلى "منشور"
